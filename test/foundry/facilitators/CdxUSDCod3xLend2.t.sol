...
contract TestCdxUSDCod3xLend2 is TestCdxUSDAndLendAndStaking {
    ...
    function testDepositsAndWithdrawals(uint256 amount) public {
        ...
        for (uint32 idx = 0; idx < commonContracts.aTokens.length - 1; idx++) {
            ...
            assertEq(_userGrainBalanceBefore + amount, commonContracts.aTokens[idx].balanceOf(address(user)));
    ...
    function testDaiBorrow() public {
        ...
        assertEq(_userAWethBalanceBefore + amount, commonContracts.aTokens[1].balanceOf(address(user)));
