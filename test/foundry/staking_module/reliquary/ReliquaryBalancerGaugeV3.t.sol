...
contract ReliquaryBalancerGaugeV3 is ERC721Holder, Test {
    ...
    function testCompareDepositAndMerge(uint256 amount1, uint256 amount2, uint256 time) public {
        amount1 = bound(amount1, 1e18, IERC20(balancerPool).balanceOf(address(this)) - 1e18);
        amount2 = bound(amount2, 1e18, IERC20(balancerPool).balanceOf(address(this)) - amount1);
    ...
    function testRelic1Level() public {
        ...
        uint256 relic2 = reliquary.createRelicAndDeposit(address(this), 0, 1e10);
    ...
    function testRelic1RewardDistribution2(uint256 seedTime) public {
        uint256 time = bound(seedTime, 1 days, 10 days);

        uint256 relic1 = 1;

        reliquary.deposit(999, 1, address(0));
        reliquary.createRelicAndDeposit(address(this), 0, 1e10);
        ...
        // test relic 1 linearity
        assertApproxEqRel(oath.balanceOf(address(11)), balance11 * 2, 1e3); // 0
        ...
        // test relic 1 linearity
        assertApproxEqRel(oath.balanceOf(address(11)), balance11 * 3, 1e3); // 0
    }
