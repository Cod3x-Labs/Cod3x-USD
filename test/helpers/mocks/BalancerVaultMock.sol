...
contract BalancerVaultMock {
    ...
    function setBalancesForTokens(bytes32 poolId, address[] memory tokens, uint256[] memory balances) external {
        _setBalancesForTokens(poolId, tokens, balances);
    }


    function _setBalancesForTokens(bytes32 poolId, address[] memory tokens, uint256[] memory balances) internal {
