...
contract MockFlashBorrower is IERC3156FlashBorrower {
    ...
    function onFlashLoan(
        ...
        if (action == Action.NORMAL) {
            ...
            (uint256 capacityBefore,) = CdxUSD(token).getFacilitatorBucket(address(_lender));
            require(capacityBefore != 0, "FlashBorrower: Zero bucket capacity before setting");

            CdxUSD(token).setFacilitatorBucketCapacity(address(_lender), 0);

            (uint256 capacityAfter,) = CdxUSD(token).getFacilitatorBucket(address(_lender));
        ...
        return
            _allowCallback ? keccak256("ERC3156FlashBorrower.onFlashLoan") : keccak256("arbitrary");
    }
